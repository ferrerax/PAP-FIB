...
int eratosthenesBlock(const int from, const int to)
{
  ...
  const int memorySize = (to - from + 0) / 2; // only odd numbers
  char * isPrime = (char *) malloc((memorySize) * sizeof(char));
  for (int i = 0; i < memorySize; i++) isPrime[i] = 1;
  ...
  int found = 0;
  for (int i = 0; i < memorySize; i++) found += isPrime[i];
...
#pragma omp taskgroup task_reduction(+:found)
{
	  for (int from = 2; from <= lastNumber; from += sliceSize) {
	    int to = from + sliceSize;
	    if (to > lastNumber) to = lastNumber;
	   #pragma omp task firstprivate(from,to) in_reduction(+:found) 
...
int main(int argc, char ** argv) {
    ...
    // Solutions count
    int count;
    count = eratosthenes(range_max, slice_size);
    STOP_COUNT_TIME;
